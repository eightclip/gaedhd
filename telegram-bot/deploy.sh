#!/usr/bin/env bash
# Run from your Mac to sync + build + restart the GaeDHD bot on the QNAP.
# Mirrors the TrakMac bot's deploy. The .env lives on the QNAP and is NOT
# overwritten by this script (rsync excludes it), so set it there once.
set -euo pipefail

REMOTE=what-server
REMOTE_DIR=/share/CACHEDEV1_DATA/Container/gaedhd-telegrambot
